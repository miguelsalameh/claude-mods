...
export type SessionOptions = {
  ...
  settings?: string
  /** more variables for the session, set on its command line */
  env?: Record<string, string>
}

export async function startSession(fixtures: Fixture[], options: SessionOptions = {}): Promise<Session> {
  const { columns = 170, rows = 80, fullscreen = false, latency, chunkSize, pluginDir = REPO, cwd = REPO, settings, env: more = {} } = options
  ...
  const name = `open-e2e-${process.pid}-${Date.now().toString(36)}`
  const debugLog = join(mkdtempSync(join(tmpdir(), 'open-e2e-')), 'debug.log')
  const env = [
    ...
    ...Object.entries(more).map(([key, value]) => `${key}=${value}`),
  ].join(' ')
